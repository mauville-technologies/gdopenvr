...
void openvr_data::release() {
	if (singleton != this) {
		...
		print_line("openvr object does not match singleton!");
	} else if (use_count > 1) {
		use_count--;
		print_line(String("Usage count decreased to ") + String::num_int64(use_count));
	} else {
		delete this;
	}
}
...
bool openvr_data::initialise() {
	...
	// init openvr
	print_line("OpenVR: initialising OpenVR context\n");
	...
	if (success) {
		...
		if (application_type == OpenVRApplicationType::OVERLAY) {
			...
			print_line("Application in overlay mode.");
		} else {
			hmd = vr::VR_Init(&error, vr::VRApplication_Scene);
			print_line("Application in scene (normal) mode.");
		}

		if (error != vr::VRInitError_None) {
			success = false;
			print_line(String("Unable to init VR runtime: ") + String(vr::VR_GetVRInitErrorAsEnglishDescription(error)));
		} else {
	...
	if (success) {
		...
		if (!render_models) {
			...
			print_line(String("Unable to get render model interface: ") + String(vr::VR_GetVRInitErrorAsEnglishDescription(error)));
		} else {
	...
	if (success) {
		if (application_type == OpenVRApplicationType::OVERLAY) {
			if (!vr::VROverlay()) {
				...
				print_line("Overlay system initialization failed. See log file for details.");
			}
		}
	}
	...
	if (success) {
		...
		if (chaperone == NULL) {
			...
			print_line("Chaperone initialization failed. See log file for details.");
		} else {
			update_play_area();
		}
	}
	...
	if (success) {
		_OS *os = _OS::get_singleton();
		Ref<_Directory> directory = memnew(_Directory);
		String exec_path = os->get_executable_path().get_base_dir().replace("\\", "/");
		String manifest_path;

		print_line(String("Exec path: ") + exec_path);
		...
		if (directory->file_exists(exec_path + "/actions/actions.json")) {
			...
			if (project_path != "") {
				print_line(String("Project path: ") + project_path);
		...
		if (manifest_path != "") {
			...
			if (err == vr::VRInputError_None) {
				print_line(String("Loaded action json from: ") + manifest_path);
			} else {
				success = false;
				print_line(String("Failed to load action json from: ") + manifest_path);
			}
		} else {
			success = false;
	...
	if (success) {
		...
		for (std::vector<action_set>::iterator it = action_sets.begin(); it != action_sets.end(); ++it) {
			...
			if (err != vr::VRInputError_None) {
				print_line(String("Failed to obtain action set handle for ") + String(it->name));
			}
		}
		...
		for (std::vector<custom_action>::iterator it = custom_actions.begin(); it != custom_actions.end(); ++it) {
			...
			if (err == vr::VRInputError_None) {
				print_line(String("Bound action ") + String(it->name) + String(" to ") + String::num_int64(it->handle));
			} else {
				print_line(String("Failed to obtain action handle for ") + String(it->name));
			}
		}
	}
...
void openvr_data::process() {
	...
	float world_scale = server->get_worldscale();
	...
	for (uint32_t i = 0; i < vr::k_unMaxTrackedDeviceCount; i++) {
		...
		if (i == 0) {
			...
			if (tracked_device_pose[i].bPoseIsValid) {
				...
				server->set_controller_transform(tracked_devices[i].tracker_id, &transform, true, true);
			}
...
int openvr_data::add_overlay(vr::VROverlayHandle_t p_new_value, RID p_viewport_rid) {
	overlay new_entry;
...
const Vector3 *openvr_data::get_play_area() const {
	return play_area;
}
...
void openvr_data::get_eye_to_head_transform(Transform *p_transform, int p_eye, float p_world_scale) {
	if (hmd == NULL) {
		return;
	}
...
int openvr_data::register_action_set(const String p_action_set) {
	...
	if (is_initialised()) {
		...
		if (err != vr::VRInputError_None) {
			print_line(String("Failed to obtain action set handle for ") + new_action_set.name);
		}
	}
...
void openvr_data::bind_default_action_handles() {
	...
	for (int i = 0; i < DAH_IN_MAX; i++) {
		...
		if (err == vr::VRInputError_None) {
			print_line(
					String("Bound action ") + String(action_path) + String(" to ") + String::num_int64(input_action_handles[i]));
		} else {
			print_line(
					String("Failed to bind action ") + String(action_path) + String(", error code: ") + String::num_int64(err));
		}
	}
	...
	for (int i = 0; i < DAH_OUT_MAX; i++) {
		...
		if (err == vr::VRInputError_None) {
			print_line(
					String("Bound action ") + String(action_path) + String(" to ") + String::num_int64(output_action_handles[i]));
		} else {
			print_line(
					String("Failed to bind action ") + String(action_path) + String(", error code: ") + String::num_int64(err));
		}
...
int openvr_data::register_custom_action(const String p_action) {
	...
	if (is_initialised()) {
		...
		if (err == vr::VRInputError_None) {
			print_line(String("Bound action ") + new_action.name + String(" to ") + String::num_int64(new_action.handle));
		} else {
			print_line(String("Failed to obtain action handle for ") + new_action.name);
		}
	}
...
Vector2 openvr_data::get_custom_analog_data(int p_action_idx, int p_on_hand) {
	if (p_action_idx < 0) {
		// we never registered our handle
		// printf("Index not setup: %i\n", p_action_idx);
		return Vector2();
	...
	return Vector2(analog_data.x, analog_data.y);
}
...
void openvr_data::attach_device(uint32_t p_device_index) {
	...
	if (p_device_index == vr::k_unTrackedDeviceIndexInvalid) {
		...
		if (device_class == vr::TrackedDeviceClass_TrackingReference) {
			...
			print_line(
					String("Found base station ") + String::num_int64(p_device_index) + String("(") + String(device_name) + String(")"));
		} else if (device_class == vr::TrackedDeviceClass_HMD) {
			// ignore any HMD
			print_line(
					String("Found HMD ") + String::num_int64(p_device_index) + String("(") + String(device_name) + String(")"));
		} else {
			int hand = 0;

			if (device_class == vr::TrackedDeviceClass_Controller) {
				print_line(
						String("Found controller ") + String::num_int64(p_device_index) + String("(") + String(device_name) + String(")"));
				...
				print_line(
						String("Found tracker ") + String::num_int64(p_device_index) + String("(") + String(device_name) + String(")"));
			}
...
void openvr_data::process_device_actions(tracked_device *p_device, uint64_t p_msec) {
			...
			switch (input_types[i]) {
				case 'b': {
					...
					if (err == vr::VRInputError_None) {
						...
						if (server.is_valid()) {
							server->set_controller_button(p_device->tracker_id, godot_index[i], action_data.bActive && action_data.bState);
						}
					}
				} break;
				case 'f': { // vector1
						...
						if (analog_data.bActive) {
							if (server.is_valid()) {
								server->set_controller_axis(
										p_device->tracker_id, godot_index[i], analog_data.x, true);
								server->set_controller_axis(
										p_device->tracker_id, godot_index[i] + 1, analog_data.x, true);
							}
						}
					}
				} break;
	...
	if (output_action_handles[DAH_OUT_HAPTIC] != vr::k_ulInvalidActionHandle) {
		if (server.is_valid()) {
			float rumble = server->get_controller_rumble(p_device->tracker_id);
...
void openvr_data::set_default_action_set(const String p_name) {
	if (is_initialised()) {
		print_line("OpenVR has already been initialised");
	} else {
		action_sets[0].name = p_name;
	}
}

const Transform *openvr_data::get_hmd_transform() const {
...
String openvr_data::get_render_model_name(uint32_t p_model_index) {
	String s;
...
bool openvr_data::_load_render_model(model_mesh *p_model) {
	...
	if (err != vr::VRRenderModelError_None) {
		print_line(String("OpenVR: Couldn''t find model for ") + String(p_model->model_name) + " (" + String::num_int64(err) + ")");
...
// requires
void openvr_data::transform_from_matrix(Transform *p_dest, vr::HmdMatrix34_t *p_matrix, float p_world_scale) {
	Basis basis;
	Vector3 origin;
	...
	origin = Vector3(p_matrix->m[0][3] * p_world_scale, p_matrix->m[1][3] * p_world_scale, p_matrix->m[2][3] * p_world_scale);
	*p_dest = Transform(basis, origin);
}

////////////////////////////////////////////////////////////////
// Convert a transform from Godot into a matrix OpenVR requires
void openvr_data::matrix_from_transform(vr::HmdMatrix34_t *p_matrix, Transform *p_transform, float p_world_scale) {
