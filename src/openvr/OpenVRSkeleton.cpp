...
void OpenVRSkeleton::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_action"), &OpenVRSkeleton::set_action);
	...
	ClassDB::bind_method(D_METHOD("set_motion_range"), &OpenVRSkeleton::set_motion_range);
	ADD_PROPERTY(PropertyInfo(Variant::INT, "motion_range", PROPERTY_HINT_NONE), "set_motion_range", "get_motion_range");

	ClassDB::bind_method(D_METHOD("get_keep_bones"), &OpenVRSkeleton::get_keep_bones);
	ClassDB::bind_method(D_METHOD("set_keep_bones"), &OpenVRSkeleton::set_keep_bones);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "keep_bones", PROPERTY_HINT_NONE), "set_keep_bones", "get_keep_bones");
...
void OpenVRSkeleton::_process(float delta) {
	...
	if (err == vr::VRInputError_NoData) {
		...
		print_line(String("Couldn't retrieve skeletal action data, err: ") + String::num_int64(err));
		return;
	}
	if (!data.bActive) {
	...
	if (bone_count == -1) {
		...
		if (err != vr::VRInputError_None) {
			...
			print_line(String("Couldn't retrieve bone count, err: ") + String::num_int64(err));
			return;
			...
			bone_count = -1; // prevent doing this again...
			print_line(String("Too many bones: ") + String::num_int64(new_bone_count));
			return;
		}
		...
		if (keep_bones && (new_bone_count == curr_bone_count)) {
			...
			if (bones == NULL) {
				bone_count = -1; // prevent doing this again...
				print_line(String("Couldn't allocate memory"));
				return;
			}
			...
			if (err != vr::VRInputError_None) {
				bone_count = -1; // prevent doing this again...
				print_line(String("Couldn't retrieve parent indices, err: ") + String::num_int64(err));
				return;
			}
			...
			if (err != vr::VRInputError_None) {
				bone_count = -1; // prevent doing this again...
				print_line(String("Couldn't retrieve reference poses, err: ") + String::num_int64(err));
				return;
			}
			...
			for (vr::BoneIndex_t i = 0; i < bone_count; i++) {
				...
				if (err != vr::VRInputError_None) {
					...
					print_line(String("Couldn't retrieve bone name, err: ") + String::num_int64(err));
				}
	...
	if (err != vr::VRInputError_None) {
		print_line(String("Couldn't retrieve skeletal bone transform data, err: ") + String::num_int64(err));
		return;
	}
